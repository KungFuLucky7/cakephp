 ...
 * @return void
 */
	public function change_account_password($userId = null) {
		if ($userId) {
			// Set a session variable for storing the account's user ID
			$_SESSION['account_id'] = $userId;
		}

		if ($this->request->is('post')) {
			$this->request->data[$this->modelClass]['id'] = $_SESSION['account_id'];
			if ($this->{$this->modelClass}->changePassword($this->request->data)) {
				$this->Session->setFlash(__d('users', 'Password changed.'));
				// we don't want to keep the cookie with the old password around
				$this->RememberMe->destroyCookie();
