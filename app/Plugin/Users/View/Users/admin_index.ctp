	...
	<table cellpadding="0" cellspacing="0">
		<tr>
			...
			<tr<?php echo $class;?>>
				...
				<td class="actions">
					<?php if ($this->Session->read('Auth.User.is_admin') || ($this->Session->check('Auth.User.role') && $this->Session->read('Auth.User.role') === 'admin')) {
									echo $this->Html->link(__d('users', 'View'), array('action' => 'admin_view', $user[$model]['id']));
									echo $this->Html->link(__d('users', 'Edit'), array('action' => 'admin_edit', $user[$model]['id']));
									echo $this->Html->link(__d('users', 'Delete'), array('action' => 'admin_delete', $user[$model]['id']), null, sprintf(__d('users', 'Are you sure you want to delete # %s?'), $user[$model]['id']));
								}
								else {
